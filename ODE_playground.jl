...
We use `ModelingToolkit.jl` to define our ODE system symbolically.
"""
...
# ╔═╡ d57eb637-29f2-4637-bf00-a16a37a662ca
md"""
Factors for modifying the parameters (see definition of `p` later)

a = $(@bind a PlutoUI.Slider( LinRange(0, 2, 21), default = 1.0, show_value = true))
``\quad``
b = $(@bind b PlutoUI.Slider( LinRange(0, 2, 21), default = 1.0, show_value = true))

C = $(@bind c PlutoUI.Slider( LinRange(0, 2, 21), default = 1.0, show_value = true))
``\quad``
d = $(@bind d PlutoUI.Slider( LinRange(0, 2, 21), default = 1.0, show_value = true))

"""

# ╔═╡ 59390064-7624-4abd-919f-ad150230f6e6
md"The following cell defines the ODE system:"

# ╔═╡ 9c1e1116-23d2-44c3-a861-6b241153665a
begin
	eqs = [
		D(x) ~ α * x + β * y,
	    D(y) ~ σ * x + δ * y
...
# ╔═╡ be5f11df-ca28-46af-948f-b6652df6b573
begin 
	@named sys = ODESystem(eqs, t) 
	sys = structural_simplify(sys) 
end

# ╔═╡ ecc8fa0d-d102-4923-80a7-75ca5f15a9ec
prob = ODEProblem(sys, u0, tspan, p);

# ╔═╡ b6a9c596-9b54-44b3-83ff-85756002ea38
sol = solve(prob, Tsit5());

# ╔═╡ 500c74ab-519a-44a4-a1aa-33257a0770b4
begin
	plot(legend_position = :topright, ylims = (-2, 2))
	plot!(sol)
end

# ╔═╡ b878dfdd-60f6-43b8-b479-e5a53f9f21ad


# ╔═╡ 46e0a244-0ee2-458f-8759-ac40606198c9
md" ## Step 2: Optimization"

# ╔═╡ 59dbe5c7-a091-45ad-adc3-d3f430f736b9
md"Load dataset"

# ╔═╡ 8cae7065-b8a5-4f44-a423-975ed7af0d24
# for external data:
# df = CSV.read("dataset.csv", DataFrame)

# analytical data
df = let
	ts = LinRange(tspan..., 10)
	xs = @. cos(ts) * exp(-0.15 * ts)
	ys = @. -sin(ts) * exp(-0.15 * ts)
	DataFrame(t = ts, x = xs, y = ys)
end

# ╔═╡ ef982401-0efb-41e8-a8b2-5c0cfc9ecce6
begin
	plot(legend_position = :topright, ylims = (-2, 2))
	title!("original ODE vs. dataset")
	plot!(sol)
	scatter!(df.t, df.x, c = :lightblue)
	scatter!(df.t, df.y, c = :orange)
end

# ╔═╡ a00d8e90-bca8-4335-8564-bf5aab89f5b5
md" ## Step 2: Optimize parameters"

# ╔═╡ 5d579381-9758-432c-b379-eef269bc4fe7
md"We now define a cost function using the loaded data."

# ╔═╡ a5b5093b-8801-4a59-a74f-2db1649de9f0
cost_function = build_loss_objective(prob, Tsit5(), 
									 L2Loss(df.t, [df.x'; df.y']),
                                     Optimization.AutoForwardDiff(),
                                     maxiters = 10000, verbose = false)

# ╔═╡ cd778dd0-d543-4a67-b1e4-8adf3691ed29
md" We can evaluate the cost function for arbitrary parameters"

# ╔═╡ f1adc74d-dfbe-43df-86fa-5fbad8861a91
p_test =  prob.p

# ╔═╡ a7db8f54-8b4d-496a-99d2-6ae80be86935
cost = cost_function( p_test )

# ╔═╡ fae6bac2-1569-43f3-8a8a-98fcdc88821c
md"Derivatives are computed automatically!"

# ╔═╡ b6b1134d-135b-44b7-ac99-f173242b6397
ForwardDiff.gradient(cost_function, p_test)

# ╔═╡ 9be6daa6-b79b-400b-b59b-4fd9ce79d94a


# ╔═╡ 5265f599-c0ad-43fd-afca-985b235f3b6a
md"Next, we can optimize the cost function!"

# ╔═╡ 0d807938-ad43-4f35-b0c4-3d97e66c1b4d
optprob = OptimizationProblem(cost_function, p_test);

# ╔═╡ c913f757-a90f-4618-83f3-d84fed60ef00
result_bfgs = solve(optprob, BFGS());

# ╔═╡ 9db7b048-fc35-4f93-8956-d17c34dad74f


# ╔═╡ 5ce73aa8-4659-4c26-8f1d-d36301152e1e
md"Optimized parameters:"

# ╔═╡ 7aef092a-1ac7-4223-8fd7-1c25b1d2f0e6
parameters(sys) .=> result_bfgs.u

# ╔═╡ 17e2b714-2a57-42d2-a573-0fdb341243e1
sol_opt = solve(prob, Tsit5(), p = result_bfgs);  # solve with new values

# ╔═╡ d81553ef-8d46-4b61-8c73-e5a88f155cf6
begin
	plot(sol_opt)
	title!("optimized ODE vs. dataset")
	plot!(sol, alpha = 0.2, c = :gray)
	scatter!(df.t, df.x, color = :blue)
	scatter!(df.t, df.y, color = :orange)
end

# ╔═╡ 1c0c8333-fe3e-44ad-a8ba-e71310b3dc48


# ╔═╡ 664cd2c6-7c0e-4f29-befb-8378c021ef94


# ╔═╡ 37941cfe-8ace-4474-8a89-5865a57a028c
md"""### Further links and documentation:

- [ModelingToolkit.jl](https://docs.sciml.ai/ModelingToolkit/stable/)
- [DiffEqParamEstim.jl](https://docs.sciml.ai/DiffEqParamEstim/stable/)
- [Optimization.jl](https://docs.sciml.ai/Optimization/stable/)
- [BlackBoxOptim.jl](https://github.com/robertfeldt/BlackBoxOptim.jl)
- [Optim](https://github.com/JuliaNLSolvers/Optim.jl/)

Local optimisation solver:
[BFGS](https://julianlsolvers.github.io/Optim.jl/stable/#algo/lbfgs/)
"""
...
git-tree-sha1 = "6dd0b91c54b5d98bb06fee51e9ca42181c427528"
